...
angular.module('customFilters').filter('sw', function() {
  return function(inputArray, startVal) {
    if (! startVal) {
      return inputArray;
    }

    var out = [];
    inputArray.forEach(function(team, pos, ary) {
      if (team.team.indexOf(startVal) === 0) {
        out.push( team );
      }
    });

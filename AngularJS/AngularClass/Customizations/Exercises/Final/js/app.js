 ...
 * Add a controller to it called StateCtrl which depends on $scope
 * Use the minification-proof style of declaring a controller
 */

angular.module( 'statesApp', [] )

  .filter('betterFilter', ['filterFilter', function(filterFilter) {
    var defaultObj = {
      name: '',
      capital: '',
      mostPopulousCity: '',
    ...
    return function(inputArray, filterObj) {
      if (! filterObj) {
        return inputArray;
      }

      var out = [];
      filterObj = _.extend( {}, defaultObj, filterObj );

      var stdObj = _.pick(filterObj, 'name', 'capital', 'mostPopulousCity');
      console.log( 'stdObj: ', stdObj );
      var firstPass = filterFilter( inputArray, stdObj );
      if (firstPass.length === 0) {
        return firstPass;
      }

      out = firstPass.filter(function(rec) {
        return (rec.population >= filterObj.population
        && rec.squareMiles >= filterObj.squareMiles)
      });
      ...
      //}
      //
      //var popVal = Number(filterObj.population) || 0;
      //var sqmVal = Number(filterObj.squareMiles) || 0;
      //// We have some filtered data
      //out = firstPass.filter(function(rec) {
      //  return (rec.population >= popVal
      //  && rec.squareMiles >= sqmVal)
      //});
      //
      return out;
  ...
  .controller( 'StateCtrl', ['$scope', function ( $scope ) {
    ...
    $scope.toggleSort = function ( field ) {
      // Did we click on some other field, or is the sort already reversed?
      // Then sort this field normally
      if ( $scope.sortField !== field || $scope.sortField[0] === '-' ) {
    ...
    // Make this JavaScript array available on the scope as "states"
    $scope.states = [
      {
